...
class UserService {
  ...
  async testingService() {
    ...
    return {
      jsonMsg: 'auth test route working'
    };
  } // signUpService
  ...
  async registerService(userObject) {
    ...
    const hash = _crypto.default.pbkdf2Sync(userObject.password, salt, 10000, 512, 'sha512').toString('hex');

    console.debug('⏳⏳⏳  [service] building new user');
    const userRecord = new _User.default({ ...userObject,
      salt: salt,
      hash: hash
    });
  ...
  async loginService(email, password) {
    ...
    let hash = _crypto.default.pbkdf2Sync(password, userRecord.salt, 10000, 512, 'sha512').toString('hex');
  ...
  async resetPassService(id, userInput) {
    ...
    await this.agendaJob.now('send-password-reset-email', {
      email: user.email
    });
  ...
  async signOutService(auth) {
    const token = auth.split(' ')[1]; // insert an add to blacklist job here
  ...
  async deleteUserService(id) {
    ...
    const del = await _User.default.findOneAndRemove({
      _id: user._id
    });
