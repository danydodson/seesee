...
then
    ...
    echo "[BUILD] Docker services_network already exists..."
fi
...
docker build --build-arg BUILD_DATE=$(date -u +'%Y-%m-%dT%H:%M:%SZ') -t danydodson/seesee_gateway_client "$dir/gateway/client"
